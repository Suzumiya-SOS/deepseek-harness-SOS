...
### Send the Harness Session id

Every provider request carries the Harness attribution headers and the route's `headers`; a request carrying a Session also carries `x-deepseek-harness-session-id` holding that Session id, which endpoints that route or cache by conversation read. Harness-owned names win a collision with a deployment header of the same name.
